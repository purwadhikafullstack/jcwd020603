...
export default function ModalKonfirmasiDeletAdmin(props) {
  ...
  const handleDelete = async (branch_id) => {
      ...
      await api.delete("/branch/", {data : { branch_id : props.dtBranch[props.number].branch_id}})
      .then((res) => {
         toast({
  ...
  return (
    <>
      <Flex padding={"20px"} flexDir={"column"} rowGap={"20px"}>
        <Flex fontWeight={"600"}>
          Apakah Anda yakin ingin menghapus data Admin : "{props.dtBranch[props.number].user_name}" dan Branch : "{props.dtBranch[props.number].Branch.branch_name}" ??
        </Flex>
